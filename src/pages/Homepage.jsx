...
const Homepage = ({ footerHeight }) => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const allRooms = details.data.flatMap(block =>
          block.floors.flatMap(floor => ({ ...floor, block_name: block.block_name }))
        );
  ...
  // ✅ New handler for navigating to the room
  const handleRoomClick = (room) => {
    try {
      const selectedBlock = block.find(
        (b) => b.block_name.toLowerCase() === room.block_name.toLowerCase()
      );
      if (!selectedBlock) {
      ...
      let foundFloor = null;
      for (const floor of selectedBlock.floors) {
        if (floor.rooms.some((r) => r.room_name === room.room_name)) {
          foundFloor = floor;
          break;
        }
      }
      if (!foundFloor) {
        toast.error("Floor not found for this room");
        return;
      }

      navigate(`/aitam/${selectedBlock.block_name}/${foundFloor._id}/rooms`, {
        state: { highlightedRoomId: room.room_id },
      });

      // navigate(`/aitam/${room.block_name}/${floorId}/rooms`, { state: { highlightedRoomId: room.room_id } });

    } catch (error) {
      console.error("Navigation error:", error);
      toast.error("Unable to navigate to the room");
    }
  };
      ...
      >
        <div className="container-fluid">
          <div
            ...
            style={{ flexWrap: 'wrap', rowGap: '10px', padding: '0 10px' }}
          >
            <div style={{ flexShrink: 0 }}>
              <img src="logo2.webp" alt="Logo" style={{
                height: '55px',
                width: 'auto',
                borderRadius: '10px',
              ...
              <div className="position-relative" ref={menuRef}>
                ...
                {showMenu && (
                  <ul className="dropdown-menu show position-absolute end-0 mt-2"
                    style={{ minWidth: '180px', borderRadius: '10px', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}
                  >
                    {access === 'super_admin' && (
                      <li>
                    ...
                    {access === 'super_admin' && (
                      <li>
                        <button className="dropdown-item text-primary" onClick={() => navigate('/globalNotice')}>
                          NoticeBoard
                        </button>
                      </li>
                    )}
            ...
            {filteredRooms.length ? (
              filteredRooms.map((room, index) => (
                <div
                  ...
                  key={index}
                  style={{ width: '100%', maxWidth: '220px', flex: '1 1 auto', cursor: 'pointer' }}
                  onClick={() => handleRoomClick(room)}
                >
                  <h5 className="text-primary">{(room.block_name || '').toUpperCase()}</h5>
                  <h6 style={{ textAlign: 'center' }}>{(room.room_name || '').toUpperCase()}</h6>
                  <p className="small">ID: {room.room_id || 'N/A'}</p>
