...
const fetchFloorData = async () => {
    try {
      ...
      const response = await axios.get(`https://dr-backend-32ec.onrender.com/block/${blockname}/floor/get-data/${floorId}`);
  ...
  const updateOccupancy = async () => {
    for (const room of roomdata) {
      ...
      if (room.occupied !== shouldBeOccupied) {
        try {
          await fetch(`https://dr-backend-32ec.onrender.com/block/floors/room/${blockId}/${floorId}/${room._id}`, {
            method: 'PUT',
...
function handleFileUpload(e) {
    ...
    const handleConfirmDelete = async () => {
      ...
      try {
        if (!blockname || !floorId) return;
          ...
          await axios.delete(`https://dr-backend-32ec.onrender.com/block/${blockId}/floor/${floorId}/room/${selectedRoom._id}`);
        }

        const updatedFloor = await axios.get(`https://dr-backend-32ec.onrender.com/block/${blockname}/floor/get-data/${floorId}`);
  ...
  try {
    ...
    // ✅ Delete the floor
    await axios.delete(`https://dr-backend-32ec.onrender.com/block/${blockId}/floor/${floorId}`);
...
const handlePauseToggle = async (roomName,pauseStatus) => {
    ...
    try {
      ...
      await axios.patch(`https://dr-backend-32ec.onrender.com/periods/pauseActivity/${blockname}/${roomName}`, {
        isPaused: newState,
      });
